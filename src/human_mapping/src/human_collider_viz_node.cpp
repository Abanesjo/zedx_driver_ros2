...
quaternionFromRotationColumns(const Vec3 &x_axis, const Vec3 &y_axis,
                              const Vec3 &z_axis) {
...
HumanColliderVizNode::HumanColliderVizNode() : Node("human_collider_viz_node") {
  input_collider_topic_ = declare_parameter<std::string>("input_collider_topic",
                                                         "/human/colliders");
  collider_markers_topic_ = declare_parameter<std::string>(
      "collider_markers_topic", "/human/collider_markers");
  collider_marker_rate_ =
      declare_parameter<double>("collider_marker_rate", 5.0);
  collision_geometry_ =
      declare_parameter<std::string>("collision_geometry", "capsules");
  sphere_interpolation_level_ =
      declare_parameter<int>("sphere_interpolation_level", 0);
  sphere_radius_gain_ = declare_parameter<double>("sphere_radius_gain", 1.0);

  if (collider_marker_rate_ <= 0.0) {
    RCLCPP_WARN(get_logger(),
                "collider_marker_rate must be positive; using 5 Hz");
    collider_marker_rate_ = 5.0;
  }
  if (collision_geometry_ != "capsules" && collision_geometry_ != "boxes" &&
      collision_geometry_ != "spheres") {
    RCLCPP_WARN(get_logger(),
                "Unsupported collision_geometry '%s'; using capsules",
  ...
  collider_sub_ = create_subscription<CapsuleArray>(
      ...
      std::bind(&HumanColliderVizNode::colliderCallback, this,
                std::placeholders::_1));
  ...
  timer_ = create_wall_timer(
      period, std::bind(&HumanColliderVizNode::timerCallback, this));

  RCLCPP_INFO(get_logger(),
              "human_collider_viz_node ready: %s -> %s (%s at %.1f Hz)",
              input_collider_topic_.c_str(), collider_markers_topic_.c_str(),
              collision_geometry_.c_str(), collider_marker_rate_);
}

void HumanColliderVizNode::colliderCallback(const CapsuleArray::SharedPtr msg) {
  latest_colliders_ = msg;
  latest_dirty_ = true;
}

void HumanColliderVizNode::timerCallback() {
...
MarkerArray
HumanColliderVizNode::makeCapsuleMarkers(const CapsuleArray &colliders) {
  MarkerArray msg;
  int id = 0;
  for (const auto &cap : colliders.capsules) {
    ...
    msg.markers.push_back(makeMarker(colliders, id++, Marker::CYLINDER, center,
                                     axisToQuaternion(a, b), diameter, diameter,
                                     shaft_length));
    msg.markers.push_back(makeMarker(colliders, id++, Marker::SPHERE, a,
                                     identityQuaternion(), diameter, diameter,
                                     diameter));
    msg.markers.push_back(makeMarker(colliders, id++, Marker::SPHERE, b,
                                     identityQuaternion(), diameter, diameter,
                                     diameter));
  }
...
MarkerArray
HumanColliderVizNode::makeBoxMarkers(const CapsuleArray &colliders) {
  MarkerArray msg;
  int id = 0;
  for (const auto &cap : colliders.capsules) {
    const Vec3 a = toVec3(cap.a);
    ...
    msg.markers.push_back(makeMarker(colliders, id++, Marker::CUBE, center,
                                     axisToQuaternion(a, b), diameter, diameter,
                                     full_length));
  }
  cleanupStale(colliders, msg, id);
...
MarkerArray
HumanColliderVizNode::makeSphereMarkers(const CapsuleArray &colliders) {
  MarkerArray msg;
  int id = 0;
  ...
  for (const auto &cap : colliders.capsules) {
    ...
    for (int i = 0; i < total_count; ++i) {
      const double t =
          total_count == 1
              ? 0.5
              : static_cast<double>(i) / static_cast<double>(total_count - 1);
      const Vec3 center = b * (1.0 - t) + a * t;
      msg.markers.push_back(makeMarker(colliders, id++, Marker::SPHERE, center,
                                       identity, diameter, diameter, diameter));
    }
  }
  cleanupStale(colliders, msg, id);
  return msg;
...
Marker
HumanColliderVizNode::makeMarker(const CapsuleArray &colliders, int id,
                                 int type, const Vec3 &center,
...
void HumanColliderVizNode::cleanupStale(const CapsuleArray &colliders,
                                        MarkerArray &msg, int next_id) {
